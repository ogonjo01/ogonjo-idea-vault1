...
const THEMES = {
  dark: {
    ...
    bg: '#060c1a',
    bgGradient:
      'radial-gradient(ellipse at 20% 0%,rgba(6,182,212,0.07) 0%,transparent 50%),radial-gradient(ellipse at 80% 100%,rgba(139,92,246,0.07) 0%,transparent 50%)',
    ...
    accent: '#06b6d4',
    accentGrad: 'linear-gradient(135deg,#06b6d4,#0891b2)',
    rowHover: 'rgba(255,255,255,0.02)',
    headerRow: 'rgba(255,255,255,0.04)',
    overlay: 'rgba(0,0,0,0.78)',
    shadow: '0 32px 80px rgba(0,0,0,0.55)',
    closeColor: '#64748b',
    themeBtnBg: 'rgba(255,255,255,0.06)',
    themeBtnBorder: 'rgba(255,255,255,0.1)',
    aiSurface: 'rgba(139,92,246,0.06)',
    aiBorder: 'rgba(139,92,246,0.2)',
    aiAccent: '#a78bfa',
    tooltipBg: '#0f172a',
    chatUserBg: 'rgba(6,182,212,0.15)',
    chatAiBg: 'rgba(255,255,255,0.04)',
    liveBg: 'rgba(255,255,255,0.02)',
    liveBorder: 'rgba(6,182,212,0.2)',
  },
  white: {
    id: 'white',
    label: '☀️ Light',
    bg: '#f8fafc',
    bgGradient:
      'radial-gradient(ellipse at 20% 0%,rgba(6,182,212,0.05) 0%,transparent 50%)',
    ...
    accent: '#0891b2',
    accentGrad: 'linear-gradient(135deg,#06b6d4,#0284c7)',
    rowHover: 'rgba(0,0,0,0.02)',
    headerRow: 'rgba(0,0,0,0.04)',
    overlay: 'rgba(0,0,0,0.5)',
    shadow: '0 32px 80px rgba(0,0,0,0.18)',
    closeColor: '#94a3b8',
    themeBtnBg: 'rgba(0,0,0,0.04)',
    themeBtnBorder: 'rgba(0,0,0,0.1)',
    aiSurface: 'rgba(99,102,241,0.04)',
    aiBorder: 'rgba(99,102,241,0.18)',
    aiAccent: '#6366f1',
    tooltipBg: '#1e293b',
    chatUserBg: 'rgba(6,182,212,0.12)',
    chatAiBg: 'rgba(0,0,0,0.04)',
    liveBg: 'rgba(0,0,0,0.02)',
    liveBorder: 'rgba(6,182,212,0.15)',
  },
  green: {
    id: 'green',
    label: '🌿 Premium',
    bg: '#071410',
    bgGradient:
      'radial-gradient(ellipse at 20% 0%,rgba(16,185,129,0.1) 0%,transparent 55%),radial-gradient(ellipse at 80% 100%,rgba(5,150,105,0.08) 0%,transparent 50%)',
    ...
    textMuted: '#4b7c6a',
    textSub: '#6ee7b7',
    tabActive: 'rgba(16,185,129,0.15)',
    tabText: '#ecfdf5',
    tabInactive: '#4b7c6a',
    ...
    rowHover: 'rgba(16,185,129,0.03)',
    headerRow: 'rgba(16,185,129,0.07)',
    overlay: 'rgba(0,0,0,0.82)',
    shadow: '0 32px 80px rgba(0,0,0,0.6)',
    closeColor: '#4b7c6a',
    themeBtnBg: 'rgba(16,185,129,0.08)',
    themeBtnBorder: 'rgba(16,185,129,0.18)',
    aiSurface: 'rgba(16,185,129,0.06)',
    aiBorder: 'rgba(16,185,129,0.22)',
    aiAccent: '#34d399',
    tooltipBg: '#022c22',
    chatUserBg: 'rgba(16,185,129,0.15)',
    chatAiBg: 'rgba(16,185,129,0.04)',
    liveBg: 'rgba(16,185,129,0.03)',
  ...
  <div style={{ display: 'flex', gap: 4 }}>
    {Object.values(THEMES).map((t) => (
      <button
        ...
        onClick={() => {
          try {
            localStorage.setItem('ogonjo_theme', t.id);
          } catch {}
          setTheme(t);
        }}
        style={{
          ...
          color: theme.id === t.id ? theme.accent : theme.textMuted,
          transition: 'all 0.15s',
        }}
      >
        {t.label}
...
const LiveFeed = ({ theme: T }) => {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.from('views').select('id,post_id,created_at,book_summaries(title,category)').gte('created_at', since).order('created_at', { ascending: false }).limit(500);
      ...
      console.error('LiveFeed fetchAll error', e);
      setAllViews([]);
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    const ch = supabase.channel('live-views-ogonjo')
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'views' }, async (payload) => {
        ...
        if (!nv?.post_id) return;
        const { data: post, error } = await supabase.from('book_summaries').select('title,category').eq('id', nv.post_id).maybeSingle();
        if (error) console.error('LiveFeed join error', error);
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 24px 12px', borderBottom: '1px solid rgba(255,255,255,0.07)', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 7 }}>
            <span style={{ fontSize: 38, fontWeight: 900, color: T.accent, letterSpacing: -1.5, lineHeight: 1 }}>{viewCount.toLocaleString()}</span>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <button onClick={() => { fetchAll(); setRefreshKey((k) => k + 1); }} title="Refresh" style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, color: 'rgba(255,255,255,0.5)', padding: '5px 11px', cursor: 'pointer', fontSize: 14 }}>↻</button>
          <button onClick={() => setFullscreen(false)} style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, color: 'rgba(255,255,255,0.5)', padding: '5px 14px', cursor: 'pointer', fontSize: 12, fontWeight: 700 }}>✕ Exit</button>
        </div>
      </div>
    ...
    <div style={{ background: T.liveBg, border: `1px solid ${T.liveBorder}`, borderRadius: 14, padding: '16px 20px', marginBottom: 14 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <button onClick={() => setFullscreen(true)} style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 7, color: T.textMuted, padding: '4px 10px', cursor: 'pointer', fontSize: 11, fontWeight: 700 }}>⛶ Fullscreen</button>
        </div>
      </div>
      ...
      {!loading && dedupedQ.length > 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4, marginTop: 10, maxHeight: 120, overflowY: 'auto' }}>
          {dedupedQ.slice(0, 5).map((v, i) => (
            <div key={v.id || i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 8px', borderRadius: 7, background: v.isNew ? 'rgba(52,211,153,0.05)' : 'transparent', border: `1px solid ${v.isNew ? 'rgba(52,211,153,0.18)' : 'transparent'}`, transition: 'all 0.3s' }}>
...
const PlatformBrief = ({ theme: T, topContent, catData, stats, prev, rawViews, period, onAskMarcus }) => {
  ...
  const audienceMap = {
    'Business Concepts': 'Strategists',
    'Business Ideas': 'Entrepreneurs',
    'Book Summary': 'Knowledge Seekers',
    'Market Analysis': 'Investors',
    'Company Profile': 'Researchers',
    'Course': 'Learners',
    'Frameworks': 'Operators',
  ...
  const runAnalysis = useCallback(async () => {
    ...
    try {
      const viewCounts = {};
      (rawViews || []).forEach((r) => {
        viewCounts[r.post_id] = (viewCounts[r.post_id] || 0) + 1;
      });
...
const AnalyticsDashboard = ({ theme: T, onAskMarcus }) => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [viewRes, likeRes, ratingRes, userRes] = await Promise.all([
      ...
      const [viewRowsA, likeRowsA, ratingRowsA, userRowsA] = [viewRes, likeRes, ratingRes, userRes].map((r) => {
        if (r.error) throw r.error;
        return r.data || [];
      });

      const [vBRes, lBRes, rBRes, uBRes] = await Promise.all([
      ...
      const prevCount = (r) => (r.error ? 0 : r.count ?? 0);

      setStats({ views: viewRowsA.length, likes: likeRowsA.length, ratings: ratingRowsA.length, newUsers: userRowsA.length });
      setPrev({ views: prevCount(vBRes), likes: prevCount(lBRes), ratings: prevCount(rBRes), newUsers: prevCount(uBRes) });
      ...
      viewRowsA.forEach((r) => {
        ...
        bkt.views[k] = (bkt.views[k] || 0) + 1;
      });
      likeRowsA.forEach((r) => {
        const k = bucketKey(r.created_at);
        bkt.likes[k] = (bkt.likes[k] || 0) + 1;
      });
      ratingRowsA.forEach((r) => {
        const k = bucketKey(r.created_at);
        bkt.ratings[k] = (bkt.ratings[k] || 0) + 1;
      });
      ...
      if (viewRowsA.length > 0) {
        ...
        if (topIds.length > 0) {
          const { data: contentDetails, error } = await supabase.from('book_summaries').select('id,title,category,views_count,likes_count,avg_rating').in('id', topIds);
          if (error) throw error;
      ...
      const [catsRes, commsRes] = await Promise.all([
  ...
  const getDrillContent = () => {
    if (period === 'Today' && selectedHour !== null) {
      const hourCounts = {};
      rawViews.forEach((r) => {
        if (new Date(r.created_at).getHours() === selectedHour) hourCounts[r.post_id] = (hourCounts[r.post_id] || 0) + 1;
      });
      const ids = new Set(Object.keys(hourCounts));
      return topContent.filter((c) => ids.has(String(c.id))).map((c) => ({ ...c, period_views: hourCounts[c.id] || 0 })).sort((a, b) => b.period_views - a.period_views);
    }
    if (period === 'Week' && selectedDay !== null) {
      const dayCounts = {};
      rawViews.forEach((r) => {
        if (new Date(r.created_at).getDay() === selectedDay) dayCounts[r.post_id] = (dayCounts[r.post_id] || 0) + 1;
      });
      const ids = new Set(Object.keys(dayCounts));
      return topContent.filter((c) => ids.has(String(c.id))).map((c) => ({ ...c, period_views: dayCounts[c.id] || 0 })).sort((a, b) => b.period_views - a.period_views);
    }
    return topContent;
  };
...
const TeamManager = ({ theme: T }) => {
  ...
  const loadUsers = useCallback(async () => {
    ...
    try {
      ...
      console.error('TeamManager error', err);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const changeRole = async (userId, newRole) => {
    ...
    try {
      ...
      console.error(err);
      alert('Could not update role. Check RLS policies.');
    } finally {
      setSaving(null);
    }
  };
...
const UserProfile = ({ onClose, onUpdated }) => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!mounted) return;
        if (error || !data) {
          const { data: upserted, error: upsertError } = await supabase.from('profiles').upsert({ id: user.id, username: '', role: 'user' }, { onConflict: 'id' }).select();
  ...
  if (loading) {
    return <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1400 }}><div style={{ color: '#94a3b8', fontSize: 13 }}>Loading…</div></div>;
  }
      ...
      <div className="modal-panel">
        ...
        <div className="profile-modal-body">
          ...
          <div className="profile-modal-info">
            ...
            <div className="profile-modal-actions" style={{ marginTop: 8 }}><button className="btn btn-outline" onClick={onClose}>Close</button></div>
          </div>
        </div>
      </div>
    </div>
